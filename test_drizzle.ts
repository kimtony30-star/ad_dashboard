import { parseCSVToRows } from './client/src/lib/csvParser.ts';
import fs from 'fs';
import { insertAdStatsBatch } from './server/dashboardDb.ts';
...
async function test() {
  ...
  // Using the ACTUAL parser used in routers.ts
  ...
  const getCol = (row, name) => { const idx = headers.indexOf(name); return idx===-1?"":((row[idx]??"").trim().replace(/^"|"$/g, "")); };
  ...
  // Try inserting 500 rows using DRIZZLE ORM
  try {
     console.log('Sending first chunk via Drizzle...');
     await insertAdStatsBatch(rows.slice(0, 500));
